...
void SaabCdmDecoder::onIgnitionFiring(uint8_t cylinderIndex, float firingAngle) {
    ...
    // action_s wraps a plain function pointer + void* arg.
    // Lambdas with captures can't convert directly, so we use static trampolines
    // with the cylinder index packed into the void* arg.
    struct CdmWindowCallbacks {
        static void openWindow(void* arg) {
            auto* self = reinterpret_cast<SaabCdmDecoder*>(
                reinterpret_cast<uintptr_t*>(arg)[0]);
            uint8_t c = static_cast<uint8_t>(
                reinterpret_cast<uintptr_t>(
                    reinterpret_cast<uintptr_t*>(arg)[1]));
            self->m_activeKnockCylinder = c;
            self->m_knock[c].windowOpen = true;
        }
        static void closeWindow(void* arg) {
            auto* self = reinterpret_cast<SaabCdmDecoder*>(
                reinterpret_cast<uintptr_t*>(arg)[0]);
            uint8_t c = static_cast<uint8_t>(
                reinterpret_cast<uintptr_t>(
                    reinterpret_cast<uintptr_t*>(arg)[1]));
            self->m_knock[c].windowOpen = false;
            if (self->m_activeKnockCylinder == c) {
                self->m_activeKnockCylinder = -1;
            }
            self->applyKnockRetard(c);
        }
    };

    // Pack this pointer and cylinder index into the per-cylinder arg arrays
    m_knock[cyl].windowOpenArg[0]  = reinterpret_cast<uintptr_t>(this);
    m_knock[cyl].windowOpenArg[1]  = static_cast<uintptr_t>(cyl);
    m_knock[cyl].windowCloseArg[0] = reinterpret_cast<uintptr_t>(this);
    m_knock[cyl].windowCloseArg[1] = static_cast<uintptr_t>(cyl);

    executor.schedule("CDM_KNOCK_OPEN",
        &m_knock[cyl].windowOpenEvent,
        now + US2NT(openDelayUs),
        action_s { CdmWindowCallbacks::openWindow,
                   reinterpret_cast<void*>(m_knock[cyl].windowOpenArg) });

    executor.schedule("CDM_KNOCK_CLOSE",
        &m_knock[cyl].windowCloseEvent,
        now + US2NT(closeDelayUs),
        action_s { CdmWindowCallbacks::closeWindow,
                   reinterpret_cast<void*>(m_knock[cyl].windowCloseArg) });
#else
