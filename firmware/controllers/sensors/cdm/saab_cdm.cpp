...
namespace {
    ...
    void knockEdgeCallback(void* /*arg*/, efitick_t timestamp) {
        // Determine edge direction by reading current pin state.
        // Active-low: pin HIGH after ISR = rising edge (pulse end),
        //             pin LOW  after ISR = falling edge (pulse start).
        bool pinHigh = palReadPad(getHwPort("cdmK", CDM_KNOCK_PIN),
                                  getHwPin("cdmK",  CDM_KNOCK_PIN));
        saabCdm.onKnockEdge(!pinHigh, timestamp);   // rising = was LOW, now HIGH
    }
...
void SaabCdmDecoder::init() {
    ...
    // Configure all three pins as inputs with no internal pull (CDM provides
    // its own pull-up resistors on all three outputs).
    ...
    // Register EXTI interrupt handlers — both edges on all three pins.
    //
    // IMPORTANT: STM32 EXTI lines are shared across all GPIO ports.
    // Each EXTI line N is triggered by ANY of PA_N, PB_N, PC_N ... PG_N etc,
    // but only ONE of those can be active at a time. Verify that CDM_KNOCK_PIN,
    // CDM_ION_13_PIN, and CDM_ION_24_PIN all have unique numeric suffixes AND
    // that those suffix numbers are not already in use by trigger inputs, VR
    // sensors, or any other efiExtiEnablePin call on your board.
    efiExtiEnablePin("CDM_KNOCK",  CDM_KNOCK_PIN,  PAL_EVENT_MODE_BOTH_EDGES,
                     knockEdgeCallback, nullptr);

    efiExtiEnablePin("CDM_ION_13", CDM_ION_13_PIN, PAL_EVENT_MODE_BOTH_EDGES,
                     ion13EdgeCallback, nullptr);

    efiExtiEnablePin("CDM_ION_24", CDM_ION_24_PIN, PAL_EVENT_MODE_BOTH_EDGES,
                     ion24EdgeCallback, nullptr);
...
void SaabCdmDecoder::onIgnitionFiring(uint8_t cylinderIndex, float firingAngle) {
    ...
    // We need a stable pointer for the scheduler callbacks.
    // Using per-cylinder scheduling_s members on the knock state struct.
...
void SaabCdmDecoder::updateOutputChannels() {
#if EFI_TUNER_STUDIO
    // Map CDM data into TunerStudio output channels.
    // Add the corresponding fields to tunerstudio_outputs.h (see comment block
    // at the bottom of this file for the exact fields to add).
    ...
    engine->outputChannels.cdmCombustionDetected[0] = isCombustionDetected(0);
    engine->outputChannels.cdmCombustionDetected[1] = isCombustionDetected(1);
    engine->outputChannels.cdmCombustionDetected[2] = isCombustionDetected(2);
    engine->outputChannels.cdmCombustionDetected[3] = isCombustionDetected(3);

    // Clear combustion flags after reading
...
//    (wherever you call engine->executor.scheduleByTimestampNt for ignition):
//
//    #include "saab_cdm.h"
//    ...
